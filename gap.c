...
typedef struct {
    ...
    int **c;    /* cost matrix c_{ij}. `double pointer` */
    int **a;    /* resource requirement matrix a_{ij} `double pointer` */
    int *b;    /* available amount b_i of resource for each agent i (int*) */
...
/**
 * @brief calculate and output cost.
 * @param (vdata) pointer of `Vdata` struct
 * @param (gapdata) `pointer of GAPdata` struct
 * @return void
 * @detail DO NOT MODIFY. See `understand_plobrems.md` for detail
 */
void recompute_cost(Vdata *vdata, GAPdata *gapdata) {
 ...
 * @param (gapdata) `pointer of GAPdata` struct
 * @return void
 * @detail data is given as `FILE*` type. but, `stdin` is used inside of this function.
 *   so, we give data by pipe with `cat` command.
 */
void read_sol(Vdata *vdata, GAPdata *gapdata) {
    int j;        /* index of jobs */
...
int main(int argc, char *argv[]) {
    ...
    /*
      ...
      compute its cost and its feasibility.
      The format of vdata.bestsol is:
    ...
    */
    ...
    // print cost to standart output.
    recompute_cost(&vdata, &gapdata);
    /// free all memory of variables
